...
struct CrossFileSearchResultsView: View {
    ...
    private func snippetText(_ snippet: CrossFileSearchSnippet) -> AttributedString {
        ...
        // Exactly ONE bold per pill — the pill's own anchor match. Pills are one-per-
        // match now, so bolds tally 1:1 with the header's match count; bolding every
        // occurrence in a shared line would double-count across its sibling pills.
