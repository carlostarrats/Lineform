...
- If a Mermaid diagram fails to render, a local, anonymous diagram log is kept on your device (the diagram source, the error, and the app version — no file names or paths). It never leaves your Mac. There is no way to send it: Lineform has no feature that transmits your document content anywhere.
