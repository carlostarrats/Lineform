...
- `WindowChromeReader.dismantleNSView` must NOT clear the window appearance. On macOS 14–26, `ChromeView` must re-assert the themed AppKit appearance on every `viewDidChangeEffectiveAppearance` and heal `window.appearance` drift through KVO. On macOS 27+, SwiftUI is the sole appearance authority: do not pin or observe AppKit appearance there, because `NSHostingView` now writes the window appearance for the root color-scheme override and a KVO write-back creates an unbounded AttributeGraph loop.
- The macOS 27 theme-authority rule has TWO guards and both are release-blocking: the default-plan `testMacOS27RepeatedThemeChangesNeverInstallAnAppKitAppearanceWriteBack` pins the no-write-back contract, while the hosted-plan `testThemeSwitchStressAcrossEditorSurfacesStaysResponsiveAndResourceBoundedOnMacOS27` exercises 300 real-window transitions across every theme and editor mode with tabs/sidebar/inspector active, enforcing forward progress, bounded resident-memory growth, and return to idle. Do not replace the hosted gate with a mock or infer its result from the unit test.
...
- `activateSelectedTab` must reconcile a CLEAN incoming tab with disk. Without it, switching to a background tab whose file changed externally shows a stale snapshot and the next keystroke autosaves over the external rewrite (silent data loss). Delayed SwiftUI text/format write-backs must also match the selected tab's document UUID; otherwise an outgoing callback can replace the incoming snapshot and reload baseline.
...
- Untitled-tab saves reached from Save-and-close, Save All, or save-then-sidebar-switch must use the tab-ID-scoped explicit `NSSavePanel` and the `SaveAsConflict` guard. The window shares one `NSDocument`; its implicit panel can retain the outgoing saved tab's filename and put that real file behind Replace even after the untitled tab becomes active.
...
- Closing the final tab must leave it in `EditorTabStore`, call `WindowCloseController.prepareForTabApprovedClose`, and dismiss the `DocumentGroup` scene with the window-specific `dismissWindow` action. Generic `dismiss` can target the just-unwinding save alert instead, while removing the tab first destroys state needed by the native close; either leaves an empty window that ignores ⌘W.
- On macOS 27, **Don't Save** for a dirty final tab must finish its sheet before scene dismissal, then use destructive `dismissWindow` only for that explicit discard. Interactive dismissal presented a second native save panel and left the old Untitled document behind. Clean and successfully saved tabs stay interactive; verify the discarded draft's provisional autosave disappears too.
- An Untitled tab's macOS draft autosave URL must stay with its `DocumentTab` across tab switches. The window shares one `NSDocument`: repointing its `fileURL` loses the outgoing draft URL, while leaving its `isDraft`/`autosavedContentsFileURL` set makes the next clean Markdown tab present a native save panel on ⌘W. Capture the draft URL before repointing, clear native draft state for the incoming tab, and remove only the captured system draft after an explicit **Don't Save** and after the document releases that URL. The installed TestFlight build with iCloud enabled is the acceptance surface; Debug without iCloud missed this regression.
...
- Development and QA builds must not accumulate duplicate Lineform Quick Look entries in macOS System Settings ▸ General ▸ Login Items & Extensions. Temporary test app copies have been reported as the cause: minimize disposable `.app` copies and reuse a stable build location where possible. Record the pre-task entries and their app paths; after build/QA work, clean up task-created temporary artifacts and their extension registrations, then verify that no additional duplicates remain compared with that baseline. Scope cleanup to confirmed task-created artifacts and registrations, preserve installed apps, user data, and required release archives, and follow the iCloud safety rules below. Never use blanket registration resets. Report any verification gaps, including unavailable baseline or UI checks and cleanup that could not be completed safely. This is a standing workflow rule only: this note does not implement automatic prevention or clean existing duplicates.
