...
- `MainMenuIconDecorator` resolves all 108 title-keyed icons by localized menu title, so its runtime language must come from `Bundle.main.preferredLocalizations`, never `Locale.language.languageCode` — the latter collapses `zh-Hans` to `zh`, matching nothing and silently losing every title-keyed icon in Chinese.
