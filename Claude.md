...
- The CJK cascade ORDER is derived from `Bundle.main.preferredLocalizations`, never hardcoded: Japanese interface → Hiragino Sans first, everything else (Simplified Chinese included) → PingFang SC first, which REPRODUCES the platform's own resolution rather than overriding it for en/de/ja/zh-Hans — zh-Hant is the one measured exception, since the platform's bare resolution there is `.PingFang UI TC` and the default branch still pins Simplified glyph forms. CoreText's implicit substitution is locale-informed, not arbitrary — a fixed Hiragino-first list rendered pure Chinese in a Japanese face for every shared Han character, the exact bug the cascade exists to prevent, and flipping it unconditionally only moves that harm onto Japanese. Assert what the cascade RESOLVES (`CTFontCreateForString` over CJK samples), never just that a list is attached: 18 attachment-only assertions shipped that regression green.
...
- `RESIGN_WITH_DEVELOPER_ID` defaults to YES and the opt-out additionally requires `ALLOW_UNSIGNED_DMG=YES`. It gates the exit-68 cert check AND the whole re-sign list, so defaulting it to NO meant the documented release command skipped both gates and still exited 0 while the release doc asserted they had run.
- `packaging/build-release.sh` removes `$APP_PATH` before `xcodebuild`. The clean-Release requirement was a rule nothing enforced; the release depended on remembering it.
- `packaging/generate-appcast.sh` writes `docs/appcast.generated.xml` and NEVER overwrites the tracked `docs/appcast.xml`. `generate_appcast` rewrites every entry's URL with one prefix, so a wholesale copy retags historical releases with the current version's download URL.
