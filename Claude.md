...
This runs `EditorDrawerMotionHostedTests` (drawer/inspector motion sampling) and `LiveReloadScrollTests` (scroll preservation across external reload — quarantined because it is the other builder of ordered-front hosted `NSWindow`s implicated in the process-exit teardown crash, and its run-loop-pumped scroll assertions are timing-sensitive). Run this plan deliberately — on a quiet machine, Xcode quit — before releases that touch editor motion, drawer/inspector presentation, or reload scroll behavior. Expectations when running it:
