...
Add the hook to your Claude Code settings. Use one of:
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "f=$(jq -r '.tool_input.file_path // empty'); case \"$f\" in *.md|*.markdown|*.txt) open -b com.lineform.app \"$f\";; esac"
          }
        ]
      }
    ]
...
files — hands it to Lineform with `open -b com.lineform.app`. It exits silently for any other
file, so it never interferes with normal work.
