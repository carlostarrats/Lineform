...
Settings, Mac App Store integration, App Intents, Quick Look, announcements, and app identity.
...
**iCloud setting behavior (2026-08-13):** **Show iCloud in sidebar** is always allowed whenever iCloud is available, including when the Lineform iCloud folder has files: hiding it changes neither the files nor their availability in a native Save panel, so an emptiness gate only traps the user in the on state. When it is off, File ▸ Save / Save As for an untitled tab proposes the workspace folder, or the user Documents folder if no workspace exists, instead of Lineform iCloud. This is only an initial location — a writer can still explicitly choose iCloud. Untitled tabs also keep their `NSDocument` change count clear while typing, so AppKit cannot auto-save or show a save panel; `DocumentTab.hasUnsavedWork` and `WindowCloseController` retain the user-initiated Save / Don't Save / Cancel close flow. Existing files continue to use native autosave.
...
- **Updates and distribution:** Lineform ships only through the Mac App Store, which owns installation and updates. The app has no self-updater, alternate package, or update UI, and release notes live in App Store Connect. Do not add an app-managed updater or a second distribution path. `ReleaseResourceTests` guards the absence of updater metadata and temporary-exception entitlements; `docs/release/app-store-release.md` is the only current release procedure.

- Standard macOS About panel showing `V1.7.2` (`AppMenuConfiguration.aboutVersionDisplay`, asserted by `AppCommandNotificationTests`). This is a HAND-MAINTAINED literal, not derived from `MARKETING_VERSION` — it silently drifted and shipped as `V1.2.0` while the app was 1.3.0 (caught 2026-07-25), then drifted to `V1.6.1` during the 1.7 bump (caught 2026-09-05). Bump it, and the test, in the same change as any version bump.
...
- **macOS 27 SwiftUI menu symbols:** macOS 27 hides symbol images in most SwiftUI menus by
  default. The app-wide main menu is unaffected because `MainMenuIconDecorator` assigns AppKit
  menu-item images directly. The Files row context menu is intentionally iconed object/destination
  vocabulary, so its action group explicitly uses `.labelStyle(.titleAndIcon)`. Keep that override
  scoped to that menu; do not globally force symbols back into every SwiftUI menu.

- **Window-menu ownership:** add **New Window** with `CommandGroup(before: .windowList)`, never a second `CommandMenu("Window")`. `DocumentGroup` already supplies AppKit's native Window menu while a document is open; declaring another top-level menu with the same title leaves two Window menus in the live menu bar. The placement keeps New Window anchored in the system menu and leaves that route available when the last document closes.

- **Edit-menu singleton cleanup:** SwiftUI can inject **Start Dictation…** and **Emoji & Symbols** more than once while rebuilding a `DocumentGroup`'s reactive command tree (live-menu dump: two and three copies respectively). `MainMenuIconDecorator.removeDuplicateSystemSingletons` collapses only the duplicate public selectors `startDictation:` and `orderFrontCharacterPalette:`, preserving the first framework-owned item in full. Do not generalize this to duplicate titles: repeated wording can represent distinct commands, and title matching would break localization.
...
- **Automation and file opening:** there is no bundled command-line tool or installer. Finder, `open -b com.lineform.app <file>`, the Claude Code hook in `docs/agent-hooks.md`, App Intents, and the app's own Open command all enter through Launch Services and receive the appropriate sandbox grant. Do not add an external helper or unused App Group entitlement.

- **Default Markdown app suggestion (2026-09-04):** Lineform declares `.md` and `.markdown` as one editable Markdown document type in `Info.plist`, but it keeps `LSHandlerRank` at `Alternate` and changes the user's preferred handler only after an explicit **Make Default** action. `DefaultMarkdownAppStore` records the first real `.md`/`.markdown` open or save and snapshots that fact at process launch, so the quiet bottom-corner invitation cannot appear during the first session; it becomes eligible on the next launch, appears once, and **Not Now** resolves it permanently. `.txt` is intentionally excluded. Settings always carries the durable **Default Markdown app** row, whether or not the one-time invitation was dismissed. Both surfaces call AppKit's public `NSWorkspace.setDefaultApplication(at:toOpen:)` API through `SystemMarkdownDefaultApplicationHandler`; there is no helper, installer, entitlement, Finder automation, or launch-time filesystem scan. Status is re-read from Launch Services whenever Lineform becomes active because Finder or another editor can change the default while Lineform is away. The card is an overlay inside `editorShell`, mutually exclusive with `AnnouncementCard`, so it does not become a full-width top row that recolors the translucent toolbar.

- **Cold launch and Dock reopen own a Lineform window (2026-09-05):** on macOS 15 and later, the `DocumentGroup` scene uses `.defaultLaunchBehavior(.suppressed)` so SwiftUI never presents its generic Open browser while launch routing settles. `LineformAppDelegate` then gives Launch Services a short 250 ms delivery window before calling `FirstLaunchIntroPresenter.openColdLaunchDocumentIfNeeded()`. That grace period is intentional: a Finder/file-intent launch can deliver its document after `applicationDidFinishLaunching`, and creating New on the next runloop still wins that race and strands the requested file behind an extra Untitled window. The helper creates a normal untitled editor only when no document arrived and the first-launch intro is not showing. macOS 14 retains the unmodified document scene because `SceneLaunchBehavior` is unavailable there, while the same guarded helper supplies the normal editor when SwiftUI leaves no document. The scene availability branch uses explicit `return` statements; this disables `SceneBuilder` for that property and permits the macOS 15 modifier to coexist with the macOS 14 return type. When Lineform remains running after its last document window closes, `applicationShouldHandleReopen` likewise creates a normal untitled document and returns `false`, preventing `DocumentGroup` from substituting its generic Open browser. It does so only when there are no visible windows and `NSDocumentController.shared.documents` is empty; an existing minimized or hidden document defers to the system so it returns without gaining a duplicate untitled document. Do not replace the shared document controller — `PlatformDocumentController` owns SwiftUI's document plumbing. Never perform a directory scan from either launch path. Folder selection belongs only to the explicit Files-sidebar **Choose** action, while Finder double-click and file intents remain the routes for opening a specific file directly.

- Apple Shortcuts / App Intents (2026-07-18): two local-first App Intents in `Lineform/App/LineformAppIntents.swift` — **New Lineform Note** (a `text` parameter → opens a note pre-filled with it) and **Open in Lineform** (an `IntentFile` → opens that Markdown/text file) — registered via `LineformAppShortcuts: AppShortcutsProvider` so they appear in the Shortcuts app, Spotlight, and Siri with no user setup. Because Raycast can run any Apple Shortcut, this also covers Raycast (no separate extension). Both are foreground intents (`static let openAppWhenRun = true`) and open through Launch Services targeting `Bundle.main.bundleURL` (`NSWorkspace.open(_:withApplicationAt:configuration:)`) so a `.txt` can't open in TextEdit and a user-picked file gets a Powerbox grant like a Finder double-click — **no new entitlement** (`user-selected.read-write` already covers the picked-file read). "New note" **stages** the text into the app's OWN container (`Application Support/Lineform/ShortcutNotes/`, always sandbox-writable, pruned after 7 days) and opens it as a real autosaving document the user can Save As anywhere; a truly untitled `DocumentGroup` document cannot be created pre-filled. **Swift 6 gotchas (build-blocking):** the `AppIntent` `title`/`description`/`openAppWhenRun` requirements must be `static let` (a `static var` trips strict-concurrency "nonisolated global mutable state"), and `@Parameter(... supportedContentTypes:)` on an `IntentFile` is macOS 15+ — omitted here since the deploy target is 14. The build runs `ExtractAppIntentsMetadata` automatically (no pbxproj phase needed) and emits `Contents/Resources/Metadata.appintents` — **but only if `AppIntents.framework` is explicitly LINKED in the app target's Frameworks build phase.** `import AppIntents` alone is not enough: without the link, `appintentsmetadataprocessor` prints "Metadata extraction skipped. No AppIntents.framework dependency found," no `Metadata.appintents` is produced, and the Shortcuts/Spotlight/Siri actions silently never register (this shipped broken until the framework was added). The intent filename sanitizer rejects only characters a filename cannot carry and preserves non-ASCII scripts; do not regress to an ASCII allowlist. Verify after any build-config change that `Contents/Resources/Metadata.appintents` exists in the built app. No AppIntents *extension* target — the intents live in the app target and run in-process on the main actor.
...
- `MainMenuIconDecorator` observes every `NSMenu` because SwiftUI builds some command menus while
  detached. App-owned context menus must therefore carry
  `MainMenuIconDecorator.excludedMenuIdentifier`; narrowing observation to the visible main menu
  breaks icon decoration when SwiftUI swaps a detached menu into place.
- The app supplies its own **Spelling and Grammar** submenu because SwiftUI does not build one for
  this command tree. Its rows need explicit icon mappings and responder-chain actions.
- `LineformAppIntents.noteTitle(from:)` rejects only characters that cannot appear in filenames
  and preserves non-ASCII scripts. An ASCII allowlist collapses many languages to “New Note” and
  creates filename collisions.
