...
- Standard macOS About panel showing `V1.6.1` (`AppMenuConfiguration.aboutVersionDisplay`, asserted by `AppCommandNotificationTests`). This is a HAND-MAINTAINED literal, not derived from `MARKETING_VERSION` — it silently drifted and shipped as `V1.2.0` while the app was 1.3.0 (caught 2026-07-25). Bump it, and the test, in the same change as any version bump.
...
- **Cold launch and Dock reopen own a Lineform window (2026-09-04):** when launch restoration and file-open delivery produce no document, `LineformAppDelegate` schedules `FirstLaunchIntroPresenter.openColdLaunchDocumentIfNeeded()` after SwiftUI installs its scenes. That creates a normal untitled document instead of leaving the user in SwiftUI's generic Open browser with no visible Lineform context. It must stay guarded by both `NSDocumentController.shared.documents.isEmpty` and the first-launch intro state: a Finder/file-intent launch already owns its document, and the intro owns the New transition after dismissal. When Lineform remains running after its last document window closes, `applicationShouldHandleReopen` likewise creates a normal untitled document and returns `false`, preventing `DocumentGroup` from substituting its generic Open browser. It does so only when there are no visible windows and `NSDocumentController.shared.documents` is empty; an existing minimized or hidden document defers to the system so it returns without gaining a duplicate untitled document. Do not replace the shared document controller — `PlatformDocumentController` owns SwiftUI's document plumbing. Never perform a directory scan from either launch path. Folder selection belongs only to the explicit Files-sidebar **Choose** action, while Finder double-click and file intents remain the routes for opening a specific file directly.
