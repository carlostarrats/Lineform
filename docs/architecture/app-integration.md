...
- **Updates: none in-app, by design (Sparkle REMOVED 2026-08-06).** Lineform ships through the Mac App Store, which supplies updates itself. Sparkle is gone entirely — the SPM dependency, `Lineform/App/AppUpdater.swift`, the "Check for Updates…" menu row, the four `SU*` `Info.plist` keys, and the `com.apple.security.temporary-exception.mach-lookup.global-name` pair in both entitlements files. **Do not re-propose an in-app updater.** It is not a preference: Sparkle's `Autoupdate`, `Updater.app` and its two XPC services are unsandboxed by design and fail App Store upload validation, there is no configuration that fixes it, and App Store apps may not ship their own update mechanism regardless. The absence is asserted, not remembered — `ReleaseResourceTests` fails if any `SU*` key or the mach-lookup exception reappears.

  What went with it: the "What's New" release-notes pane, which was Sparkle's UI. Per-release notes were authored as `docs/release-notes/Lineform-<version>.html` and `generate_appcast --embed-release-notes` inlined them into the appcast `<description>`. That whole pipeline was deleted with Sparkle: `packaging/generate-appcast.sh`, `verify-update.sh`, `build-release.sh`, `build-dmg.sh`, `notarize-dmg.sh`, `docs/appcast.xml`, `docs/release-notes/`, and the Release Build workflow. Release notes now live on the App Store Connect record. Consequence worth knowing: `docs/appcast.xml` is gone from `main`, so every already-installed DMG copy of Lineform checks its feed once, gets a 404, and stays on its current version permanently — those users are reachable only through the website and the announcements card. See `docs/release/app-store-release.md`.
...
- **Command line tool: REMOVED 2026-08-06.** The bundled `lineform` helper (`Contents/Helpers/lineform`), its shared logic (`Lineform/CommandLineTool/LineformCommandLine.swift`), its entry point (`HelperTool/main.swift`), the **Lineform → Install Command Line Tool…** installer, and the `~/Library/Application Support/Lineform/Piped/` staging directory are all gone. It opened files (`lineform file.md`) or stdin (`… | lineform -`) via `open`, and shipped only in packaged release builds.

  **Do not rebuild it, and do not re-propose the URL-scheme workaround.** Every executable in an App Store bundle must be sandboxed, and a sandboxed helper cannot hand a FILE to the app: `NSWorkspace` by app path, `open -a`, `open -b`, and a file-URL default-handler open all fail regardless of location, signing, or file entitlements. A custom `lineform://` scheme WOULD launch the app — scheme opening is permitted from the sandbox — but a path delivered that way carries no sandbox extension, so the app could only read files inside a workspace folder it already holds a bookmark for. A CLI that works in one folder and silently fails everywhere else was judged worse than no CLI. Full investigation: `docs/superpowers/specs/2026-07-29-sandboxed-cli-helper-design.md`.

  Consequences elsewhere: the unsandboxed helper is also why `Contents/Helpers` forced the clean-`Release/Lineform.app` rule (the rule stays, the reason changed), and the Claude Code hook in `docs/agent-hooks.md` now uses `open -b com.lineform.app` directly — it always carried that fallback, so the recipe lost a line rather than a capability. The App Group `group.TV4QZT7A7X.com.lineform` on the App ID was added for the abandoned sandboxed helper and is deliberately NOT declared by the app; do not add an entitlement with no consumer.
...
> **HISTORICAL — the machinery below no longer exists.** Direct/DMG distribution, Sparkle, and the
> `lineform` helper were deleted on 2026-08-06; `packaging/` now holds only the two localization
> extraction scripts. This section is kept because the *failure modes* are what App Store
> submission has to avoid too (a cert absent from the profile, a nested bundle signed with the
> wrong identity, a verifier that passes without checking the artifact that ships) — not because
> any of these scripts or flags are still real. Do not act on the file paths or variables named
> here.
