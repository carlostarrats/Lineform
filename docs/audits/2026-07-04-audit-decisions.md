...
- [x] 11. Task 5 — sidebar scan — done 2026-07-05, branch `worktree-task5-sidebar-scan` (fresh worktree, off `origin/main` = `ac5a039`, all prior work merged). Suite **492/0** (default plan minus the 2 TCC-triggering meta classes — see the ⚠️ note below), reviewed (subagent, adversarial — clean, no blocking/should-fix; 1 optional nit applied). Spec `docs/superpowers/specs/2026-07-05-sidebar-scan-debounce-design.md`, plan `docs/superpowers/plans/2026-07-05-sidebar-scan-debounce.md`. **Fix shape chosen: Option 2 (trailing debounce) only** — the audit's own "simple, low-risk, could kill most of the hitch," and per the task's "skip the fanciest optimizations in v1." Shipped:
  ...
  - **Deferred by decision (v1):** Option 1 off-main (highest concurrency risk in this incident-prone area — the sanctioned follow-up **if** a huge-tree *pause-settle* is ever felt; cheap because `items(in:)` is already a pure static fn), Option 3 incremental scope-splice (the "subtle, hard-to-repro" complexity), Option 4 cap micro-opt (recursion is already cap-first).
  - ⚠️ **NOT exercised / needs your hands:** (1) **Felt-smoothness QA** — automation can't type into the app, and the symptom is a *felt* typing hitch on a many-file workspace; the unit tests prove the debounce *mechanism* (deferral / eventual correctness / cancel), but the felt result must be confirmed in-app by you on a large workspace. (2) **Full-suite completion** — the 2 skipped classes (`ReleaseResourceTests`, `TestPlanGuardTests`) read repo config files under `~/Documents`, so the re-signed test host's TCC Documents prompt **hangs them while unattended** (exactly the CLAUDE.md TCC caveat; I was told you were away and couldn't click Allow). They are unrelated to this change and were green in the prior audit (505/0). Please run the full default plan once (click **Allow** on the Documents prompt) to confirm those 2 classes + the felt typing smoothness before merge.
