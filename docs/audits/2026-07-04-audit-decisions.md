...
- [x] 11. Task 5 — sidebar scan — done 2026-07-05, branch `worktree-task5-sidebar-scan` (fresh worktree, off `origin/main` = `ac5a039`, all prior work merged). Suite **507/0** (full default plan — user was back to click Allow on the TCC Documents prompt, so the 2 repo-file-reading meta classes ran too; during the unattended window it was 492/0 skipping just those 2, see the ⚠️ note), reviewed (subagent, adversarial — clean, no blocking/should-fix; 1 optional nit applied). Spec `docs/superpowers/specs/2026-07-05-sidebar-scan-debounce-design.md`, plan `docs/superpowers/plans/2026-07-05-sidebar-scan-debounce.md`. **Fix shape chosen: Option 2 (trailing debounce) only** — the audit's own "simple, low-risk, could kill most of the hitch," and per the task's "skip the fanciest optimizations in v1." Shipped:
  ...
  - ✅ **Full default plan now confirmed 507/0** (the 2 repo-file-reading meta classes `ReleaseResourceTests` + `TestPlanGuardTests` ran once the user was back to Allow the TCC Documents prompt — unrelated to this change, always expected to pass).
  - ⚠️ **Still needs your hands — felt-smoothness QA:** automation can't type into the app, and the symptom is a *felt* typing hitch on a many-file workspace. The unit tests prove the debounce *mechanism* (deferral / eventual correctness / cancel), but the felt result must be confirmed in-app by you on a large workspace before merge.
